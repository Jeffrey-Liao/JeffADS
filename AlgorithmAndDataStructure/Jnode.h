...
namespace jeff
{
	...
	public:
		...
		T addEqual(const T existData)
		{
			return this->data += existData;
		}
		T addEqual(const Node& existNode)
		{
			return this->data += existNode.data	;
		}
		T add(const T existData)
		{
			return this->data + existData;
		}
		T add(const Node& existNode)
		{
			return this->data + existNode.data;
		}
	...
	public:
		...
		LinkNode& operator()(int distance)
		{
			LinkNode* temp = this;
	...
	public:
		...
		DoubleLinkNode(DoubleLinkNode* father, DoubleLinkNode* Son, const T newData) :next(Son), Node<T>(newData)
		{
			if (father != NULL)
				father->next = this;
			if (Son != NULL)
		...
		{
			DoubleLinkNode* tempNext = this->next;
			this->next  = &existNode;
			existNode.prev = this;
			if (!existNode.last())
			{
				DoubleLinkNode* temp = existNode.next;
